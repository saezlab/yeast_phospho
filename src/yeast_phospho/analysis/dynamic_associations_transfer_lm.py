...
k_activity_dyn_ng_gsea = read_csv('%s/tables/kinase_activity_dynamic_no_growth.tab' % wd, sep='\t', index_col=0)
...
plt.savefig('%s/reports/lm_dynamic_boxplots.pdf' % wd, bbox_inches='tight')
plt.close('all')
...
g.fig.subplots_adjust(wspace=.05, hspace=.2)
sns.despine(trim=True)
plt.savefig('%s/reports/lm_dynamic_metabolites.pdf' % wd, bbox_inches='tight')
plt.close('all')
print '[INFO] Plot done'
...
lm_f_res.sort('coef_abs', ascending=False).to_csv('%s/tables/metabolites_kinases_interactions_lm.csv' % wd, index=False)
...
plt.savefig('%s/reports/lm_dynamic_roc.pdf' % wd, bbox_inches='tight')
plt.close('all')
print '[INFO] Plot done'
...
plt.gcf().set_size_inches(3, 3)
plt.savefig('%s/reports/lm_dynamic_prc.pdf' % wd, bbox_inches='tight')
plt.close('all')
print '[INFO] Plot done'
...
plt.savefig('%s/reports/lm_dynamic_heatmap.pdf' % wd, bbox_inches='tight')
plt.close('all')
print '[INFO] Plot done'
