...
# GSEA
xs = read_csv('%s/tables/tf_activity_dynamic_no_growth.tab' % wd, sep='\t', index_col=0)
xs = xs[xs.std(1) > .1]
...
plt.savefig('%s/reports/lm_dynamic_boxplots_tfs.pdf' % wd, bbox_inches='tight')
plt.close('all')
...
g.fig.subplots_adjust(wspace=.05, hspace=.2)
sns.despine(trim=True)
plt.savefig('%s/reports/lm_dynamic_metabolites_tfs.pdf' % wd, bbox_inches='tight')
plt.close('all')
print '[INFO] Plot done'
...
lm_f_res.sort('coef_abs', ascending=False).to_csv('%s/tables/metabolites_tfs_interactions_lm.csv' % wd, index=False)
...
plt.savefig('%s/reports/lm_dynamic_roc_tfs.pdf' % wd, bbox_inches='tight')
plt.close('all')
print '[INFO] Plot done'
...
plt.gcf().set_size_inches(3, 3)
plt.savefig('%s/reports/lm_dynamic_prc_tfs.pdf' % wd, bbox_inches='tight')
plt.close('all')
print '[INFO] Plot done'
...
lm_res_top_features.to_csv('%s/tables/metabolites_top_tfs_interactions_lm.csv' % wd, index=False)

t_matrix = pivot_table(lm_res_top_features, index='Metabolites', columns='Transcription-factors', values='coef')
t_matrix = t_matrix.loc[:, t_matrix.std() > .025]
...
plt.savefig('%s/reports/lm_dynamic_heatmap_tfs.pdf' % wd, bbox_inches='tight')
plt.close('all')
print '[INFO] Plot done'
