...
for k_file, m_file, growth in datasets_files:
    ...
    k_activity, r_activity = k_activity[strains], r_activity[strains].dropna()
    ...
    info_table['adjpvalue'] = multipletests(info_table['pvalue'], method='fdr_bh')[1]
    info_table['log_adjpvalue'] = [-np.log10(i) for i in info_table['adjpvalue']]
    ...
    info_table['TP'] = [int(i in db) for i in zip(info_table['kinase'], info_table['reaction'])]
    ...
    M = {(k, m) for k, m in set(zip(info_table['kinase'], info_table['reaction'])) if k in db_proteins}
    n = M.intersection(db)
    ...
    for threshold in thresholds:
        N = info_table.loc[info_table['log_pvalue'] > threshold]
