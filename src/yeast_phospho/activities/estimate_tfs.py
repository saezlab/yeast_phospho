...
trans = read_csv('%s/tables/transcriptomics_steady_state.tab' % wd, sep='\t', index_col=0).dropna()[ko_strains]
...
# Regress-out growth
tf_activity = DataFrame({k: regress_out(growth[ko_strains], tf_activity.ix[k, ko_strains]) for k in tf_activity.index}).T
